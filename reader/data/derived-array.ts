...
export function createSourceArray(arr = undefined) {
  ...
  arr.deriveArray = function(mapFun) {
    ...
    derArr.derPush = function(item) {
      this.push(this.mapFun(item));
    };
    derArr.derInsert = function(i, item) {
      this.splice(i, 0, this.mapFun(item));
    };
...
export function createSortedDerivedArray(mapFun, isBeforeSortFun, filterFun) {
  ...
  sortedArr.derPush = function(item) {
    if (!filterFun(this, item)) {
      return;
    }
    let mapped = this.mapFun(item);
    let i = this.findIndex(
      function(i) {
        return this.isBeforeSortFun(mapped, i);
      }.bind(this)
    );
    if (i === -1) {
      i = this.length;
